...
void showing_status(char * branch){
        ...
        if(init(1)==1){
            ...
            if(strcmp(argv[2],"HEAD")==0){
                ...
                if(last_barnch_commit<0){
                    char x[1000];
                    char branch[1000];
                    file=fopen(".sag\\curr_branch.txt","r");
                    fgets(x,sizeof(x),file);
                    sprintf(branch,".sag\\all_branches\\%s,txt",x);
                    file=fopen(branch,"r");
                    fgets(x,sizeof(x),file);
                    checkout_with_num(x,0);
                }
                else{
                    delete_all_files();
                    file=fopen(".sag\\curr_commit.txt","w");
                ...
                if(last_barnch_commit<0){
                    ...
                    printf("%s\n",argv[2]);
                    fclose(file);
                    sprintf(branch,".sag\\all_branches\\%s.txt",argv[2]);
                    file=fopen(branch,"r");
                    fgets(x,sizeof(x),file);
                    printf("%s",x);
                    checkout_with_num(x,0);
...
//     file=fopen(person,"w");
//     fprintf(stdout, "commit successfully with commit ID %s \ntime : %s", tmp,ctime(&curr));
//     fprintf(file,"%s",find_the_last_person());
//     fclose(file);
//     file=fopen(time1,"w");
//     fprintf(file,"%s",ctime(&curr));
//     fclose(file);
//     file=fopen(files,"w");
//     chdir(".sag\\files");
//     DIR *dir=opendir(".");
...
//                 else{
//                     char commit[1000];
//                     FILE *file=fopen(".sag\\all_branches.txt","a");
//                     fprintf(file,"%s\n",argv[2]);
//                     fclose(file);
//                     file=fopen(".sag\\curr_commit.txt","r");
//                     fgets(commit,sizeof(commit),file);
//                     printf("%s\n",commit);
//                     char filepath[1000];
//                     fclose(file);
//                     sprintf(filepath,".sag\\commits\\%s\\branch.txt",commit);
