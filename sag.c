...
void replace(char *name,char*message,int num,char *commit_num){
    ...
    sprintf(system_command,"rmdir /s /q. sag\\tags\\%s",name);
...
void showing_status(char * branch){
    ...
    if(last<0){
        ...
        sprintf(branch_path,".sag\\all_branches\\%s.txt",branch);
        ...
        if(init(1)==1){
            ...
            if(argc==2){
                ...
                if(exist){
                    ...
                    fprintf(file,"%d",finding_last_commit_of_a_branch("master"));
                }
            }
        }
    }
